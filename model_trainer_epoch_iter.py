...
class UnetTrainer():
    ...
    def initialize_loss_function(self):
        ...
        if self.deep_supervision:
            ...
            loss_weights = np.array([1 / (2 ** i) for i in range(self.num_res_supervision)])[::-1] #-1 because we don't use bottleneck layer. reverse bc the high res ones are important
    ...
    def train(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            # print("define generatior: ", time()-self.epoch_start_time )
    ...
    def predict_heatmaps_and_coordinates(self, data_dict,  return_all_layers = False, resize_to_og=False,):
        ...
        if resize_to_og:
            final_heatmap = Resize(self.orginal_im_size, interpolation=  InterpolationMode.BICUBIC)(final_heatmap)
    ...
    def set_training_dataloaders(self):
        ...
        self.valid_dataloader = DataLoader(valid_dataset, batch_size=len(valid_dataset), shuffle=False, worker_init_fn=UnetTrainer.worker_init_fn )
        # self.train_dataloader = DataLoader(train_dataset, batch_size=self.model_config.SOLVER.DATA_LOADER_BATCH_SIZE, shuffle=True, num_workers=num_workers_cfg,persistent_workers=True, worker_init_fn=UnetTrainer.worker_init_fn )
        # self.valid_dataloader = DataLoader(valid_dataset, batch_size=len(valid_dataset), shuffle=False, num_workers=num_workers_cfg,persistent_workers=True, worker_init_fn=UnetTrainer.worker_init_fn )
