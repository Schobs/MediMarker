...
class EnsembleUncertainties:
    ...
    def ensemble_inference_with_uncertainties(self, evaluation_logs):
        ...
        all_ensemble_results_dicts = {
            uncert_key: [] for uncert_key in self.uncertainty_keys
        }
        ind_errors = {
            uncert_key: [[] for x in range(len(self.landmarks))]
            for uncert_key in self.uncertainty_keys
        }
        ...
        for uid_, results_dict_list in sorted_dict_by_uid.items():
            ...
            for k_ in list(all_ensemble_results_dicts.keys()):
                ...
                for sample_info_key in evaluation_logs["sample_info_log_keys"]:
                    ensemble_results_dict[k_][sample_info_key] = individual_results[0][
                        sample_info_key
                    ]
            ...
            if calc_error:
                assert all(
                    [
                        ...
                        for y in [
                            (
                                ind_res["target_coords"]
                                == individual_results[0]["target_coords"]
                            )
                            for ind_res in individual_results
                        ]
                    ]
                ), "Target coords are not the same for all models in the ensemble"
            ...
            if "smha" in self.uncertainty_keys:
                ensemble_results_dict, ind_errors = self.calculate_smha(
                    individual_results, ensemble_results_dict, ind_errors, calc_error
                )

            ################## 2) E_CPV ###########################
            if "ecpv" in self.uncertainty_keys:
                ensemble_results_dict, ind_errors = self.calculate_ecpv(
                    individual_results,
                    ensemble_results_dict,
                    ind_errors,
                    target_coords,
            ...
            if "emha" in self.uncertainty_keys:
                ensemble_results_dict, ind_errors = self.calculate_emha(
                    individual_results,
                    ensemble_results_dict,
                    ind_errors,
                    target_coords,
    ...
    def calculate_smha(
        self, individual_results, ensemble_results_dict, ind_errors, calc_error
    ):
        ...
        ensemble_results_dict["smha"]["predicted_coords"] = individual_results[
            self.smha_choice_idx
        ]["predicted_coords"]
        ensemble_results_dict["smha"]["uncertainty"] = individual_results[
            self.smha_choice_idx
        ]["hm_max"]

        # Save inference resolution results for each landmark individually
        for c_idx, pred_coord_t in enumerate(
            individual_results[self.smha_choice_idx]["predicted_coords"]
        ):
            ensemble_results_dict["smha"]["predicted_L" + str(c_idx)] = pred_coord_t
            ensemble_results_dict["smha"][
                "uncertainty_L" + str(c_idx)
            ] = ensemble_results_dict["smha"]["uncertainty"][c_idx][0]

        ####Calculate the error if annotation available
        if calc_error:
            # Error for inference resolution
            ensemble_results_dict["smha"]["ind_errors"] = individual_results[
                self.smha_choice_idx
            ]["ind_errors"]
            ensemble_results_dict["smha"]["Error All Mean"] = np.mean(
                individual_results[self.smha_choice_idx]["ind_errors"]
            )
            ensemble_results_dict["smha"]["Error All Std"] = np.std(
                individual_results[self.smha_choice_idx]["ind_errors"]
            )

            for cidx, p_coord in enumerate(
                individual_results[self.smha_choice_idx]["ind_errors"]
            ):
                ensemble_results_dict["smha"]["L" + str(cidx)] = p_coord
                ensemble_results_dict["smha"][
                    "uncertainty L" + str(cidx)
                ] = individual_results[self.smha_choice_idx]["hm_max"][cidx]
    ...
    def calculate_ecpv(
        ...
        # Calculate the E-CPV
        average_coords = np.mean(
            [dic["predicted_coords"] for dic in individual_results], axis=0
        )
        ...
        for coord_idx, coord in enumerate(average_coords):
            all_coord_vars.append(
                np.mean(
                    [
                        abs(np.linalg.norm(coord - x))
                        for x in [
                            dic["predicted_coords"][coord_idx]
                            for dic in individual_results
                        ]
                    ]
                )
            )
        ...
        # Save inference resolution results for each landmark individually
        for c_idx, pred_coord_t in enumerate(
            ensemble_results_dict["ecpv"]["predicted_coords"]
        ):
            ensemble_results_dict["ecpv"]["predicted_L" + str(c_idx)] = pred_coord_t
            ensemble_results_dict["ecpv"][
                "uncertainty_L" + str(c_idx)
            ] = ensemble_results_dict["ecpv"]["uncertainty"][c_idx]

        ####Calculate the error if annotation available
        if calc_error:
            # Error for inference resolution
            ecpv_errors = np.linalg.norm(
                ensemble_results_dict["ecpv"]["predicted_coords"] - target_coords,
                axis=1,
            )
    ...
    def calculate_emha(
        ...
        ensemble_map = torch.zeros(
            (
                1,
                all_ind_heatmaps[0].shape[0],
                all_ind_heatmaps[0].shape[1],
                all_ind_heatmaps[0].shape[2],
            )
        )
        ...
        for model_idx, per_model_hms in enumerate(all_ind_heatmaps):
            ...
            assert all(
                [x.shape == per_model_hms[0].shape for x in per_model_hms]
            ), "Output ensemble heatmaps are not the same size!"
        ...
        # Save inference resolution results for each landmark individually
        for c_idx, pred_coord_t in enumerate(
            ensemble_results_dict["emha"]["predicted_coords"]
        ):
            ensemble_results_dict["emha"]["predicted_L" + str(c_idx)] = pred_coord_t
            ensemble_results_dict["emha"][
                "uncertainty_L" + str(c_idx)
            ] = ensemble_results_dict["emha"]["uncertainty"][c_idx][0]

        if calc_error:
            # Error for inference resolution
            emha_errors = np.linalg.norm(
                ensemble_results_dict["emha"]["predicted_coords"] - target_coords,
                axis=1,
            )
            ...
            for cidx, p_coord in enumerate(emha_errors):
                ...
                ensemble_results_dict["emha"][
                    "uncertainty L" + str(cidx)
                ] = ensemble_results_dict["emha"]["uncertainty"][cidx]
