...
class PHDResNetTrainer(NetworkTrainer):
    """Class for the phdnet trainer stuff."""

    def __init__(self, **kwargs):

        super(PHDResNetTrainer, self).__init__(**kwargs)
    ...
    def initialize_network(self, num_landmarks):
        ...
        self.network = PHDResNet(ResNetBlock, layers=[3, 4, 6, 4],
                                 branch_scheme=self.branch_scheme, num_landmarks=self.num_landmarks)
    ...
    def get_coords_from_heatmap(self, model_output, original_image_size):
        ...
        for i in range(self.num_landmarks):
            ...
            if pred_coords == None:
                ...
                extra_info["final_heatmaps"] = torch.cat(
                    (extra_info["final_heatmaps"], smoothed_candidate_maps), dim=1)
                extra_info["debug_candidate_smoothed_maps"] = torch.cat(
                    (extra_info["debug_candidate_smoothed_maps"], smoothed_candidate_maps), dim=1)
