...
class ViT(nn.Module):
    def __init__(self, *, num_landmarks, branch_scheme, image_size, patch_size, dim, depth, heads, mlp_dim, pool = 'cls', channels = 1, dim_head = 64, dropout = 0., emb_dropout = 0.):
        super().__init__()
        ...
        self.layer_reg = nn.Sequential(
            nn.Conv2d(768, 64, kernel_size=1, stride=1, bias=True),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, num_landmarks * 2, kernel_size=1, stride=1, bias=True)
        )
        ...
        self.layer_class = nn.Sequential(
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, num_landmarks, kernel_size=1, stride=1, bias=True))
    ...
    def forward(self, img):
        ...
        #SHAPE IS [12, 257, 1024]
        
        x = x[:, 1:] #discard the classification token
        x = x.permute(0, 2, 1)
