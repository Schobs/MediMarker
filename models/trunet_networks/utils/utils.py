...
from .misc import ensure_tuple, save_obj, set_determinism
from .module import look_up_option, pytorch_after
from .type_conversion import convert_to_tensor
...
def look_up_named_module(name: str, mod, print_all_options=False):
    ...
    for n in name_str.split("."):
        if n.isdigit():
            ...
            n = look_up_option(n, {item[0] for item in mod.named_modules(
            )}, default=None, print_all_options=False)
            if n is None:
                return None
            mod = getattr(mod, n)
    return mod
...
def normalize_transform(
    ...
    """
    shape = convert_to_tensor(
        shape, torch.float64, device=device, wrap_sequence=True, track_meta=False)
    norm = shape.clone().detach().to(dtype=torch.float64,
                                     device=device)  # no in-place change
    if align_corners:
        norm[norm <= 1.0] = 2.0
        norm = 2.0 / (norm - 1.0)
        norm = torch.diag(
            torch.cat((norm, torch.ones((1,), dtype=torch.float64, device=device))))
...
def to_norm_affine(
    ...
    if not isinstance(affine, torch.Tensor):
        raise TypeError(
            f"affine must be a torch.Tensor but is {type(affine).__name__}.")
    ...
    if sr != len(src_size) or sr != len(dst_size):
        raise ValueError(
            f"affine suggests {sr}D, got src={len(src_size)}D, dst={len(dst_size)}D.")

    src_xform = normalize_transform(
        src_size, affine.device, affine.dtype, align_corners, zero_centered)
    dst_xform = normalize_transform(
        dst_size, affine.device, affine.dtype, align_corners, zero_centered)
...
def pixelshuffle(x: torch.Tensor, spatial_dims: int, scale_factor: int) -> torch.Tensor:
    ...
    output_size = [batch_size, org_channels] + \
        [d * factor for d in input_size[2:]]
...
def copy_model_state(
    ...
    to_skip = {s_key for s_key in src_dict if exclude_vars and re.compile(
        exclude_vars).search(s_key)}
    ...
    for s in mapping if mapping else {}:
        ...
        if dst_key in dst_dict and dst_key not in to_skip:
            if dst_dict[dst_key].shape != src_dict[s].shape:
                warnings.warn(
                    f"Param. shape changed from {dst_dict[dst_key].shape} to {src_dict[s].shape}.")
...
def convert_to_torchscript(
    ...
    **kwargs,
    ...
    if verify:
        ...
        inputs = [i.to(device) if isinstance(
            i, torch.Tensor) else i for i in inputs]
        ts_model = torch.jit.load(
            filename_or_obj) if filename_or_obj is not None else script_module
        ...
        for r1, r2 in zip(torch_out, torchscript_out):
            if isinstance(r1, torch.Tensor) or isinstance(r2, torch.Tensor):
                assert_fn = torch.testing.assert_close if pytorch_after(
                    1, 11) else torch.testing.assert_allclose
    ...
    # no "." in module name, do the actual replacing
        ...
        if strict_match:
            ...
            for mod_name, _ in parent.named_modules():
                if name in mod_name:
                    _replace_modules(parent, mod_name, deepcopy(
                        new_module), out, strict_match=True)
...
def replace_modules_temp(
    ...
    try:
        # replace
        _replace_modules(parent, name, new_module, replaced,
                         strict_match, match_device)
        yield
    finally:
        # revert
        for name, module in replaced:
            _replace_modules(parent, name, module, [],
                             strict_match=True, match_device=match_device)
