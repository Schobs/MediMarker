...
UNSUPPORTED_TYPES = {np.dtype("uint16"): np.int32, np.dtype(
    "uint32"): np.int64, np.dtype("uint64"): np.int64}
...
def convert_to_tensor(
    ...
    if isinstance(data, np.ndarray):
        ...
        list_ret = [convert_to_tensor(
            i, dtype=dtype, device=device, track_meta=track_meta) for i in data]
        return _convert_tensor(list_ret, dtype=dtype, device=device) if wrap_sequence else list_ret
    elif isinstance(data, tuple):
        tuple_ret = tuple(convert_to_tensor(
            i, dtype=dtype, device=device, track_meta=track_meta) for i in data)
...
def convert_to_cupy(data: Any, dtype: np.dtype | None = None, wrap_sequence: bool = False, safe: bool = False) -> Any:
    ...
    if not isinstance(data, cp.ndarray):
        raise ValueError(
            f"The input data type [{type(data)}] cannot be converted into cupy arrays!")
...
def convert_data_type(
    ...
    if issubclass(output_type, np.ndarray):
        data_ = convert_to_numpy(
            data, dtype=dtype_, wrap_sequence=wrap_sequence, safe=safe)
        return data_, orig_type, orig_device
    elif has_cp and issubclass(output_type, cp.ndarray):
        data_ = convert_to_cupy(
            data, dtype=dtype_, wrap_sequence=wrap_sequence, safe=safe)
        return data_, orig_type, orig_device
...
def convert_to_dst_type(
    ...
    device = dst.device if device is None and isinstance(
        dst, torch.Tensor) else device
    if dtype is None:
        dtype = dst.dtype
    ...
    if isinstance(dst, monai.data.MetaTensor):
        ...
        if not isinstance(src, monai.data.MetaTensor):
            # converting a non-meta tensor to a meta tensor, probably take the metadata as well.
            copy_meta = True
...
def get_dtype_bound_value(dtype: DtypeLike | torch.dtype) -> tuple[float, float]:
    ...
    if dtype in UNSUPPORTED_TYPES:
        ...
        is_floating_point = get_equivalent_dtype(
            dtype, torch.Tensor).is_floating_point
