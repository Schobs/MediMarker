...
def success_detection_rate(sample_dicts, threshold):
    ...
    perc_over_all = len(images_within_thresh_all) / \
        ((total_samples * total_landmarks))

    # Calc % of samples over threshold, calculate per landmark
    perc_over_ind = [
        len(x) / total_samples for x in images_within_thresh_per_lm]
...
def nlpd_evaluation(list_of_dicts):
    ...
    for sample in list_of_dicts:
        ...
        if isinstance(sample["cov"], float):
        ...
        elif len(sample["cov"]) == 2:
            c_kern = sample["cov"][0].replace('\n', '')
            # Use regular expressions to replace spaces with commas
            c_kern = re.sub("[ ]{1,}", ",", c_kern)
            c_kern = c_kern.replace(',]', ']')
            c_kern = c_kern.replace('[,', '[')

            c_kern = np.array(eval(c_kern))

            c_like = sample["cov"][1].replace(' ', ', ')
            c_like = re.sub(r'(?<!\d)(\s?)(,)(\s?)(?!\d)', r'\1\3', c_like)
            c_like = c_like.replace('. ', ', ')

            c_like = np.array(eval(c_like))

            pred_cov = np.array(c_kern + [[c_like[0], 0], [0, c_like[1]]])
            pred_mean = np.expand_dims(pred_mean, axis=0)
        else:
            raise ValueError("Covariance matrix value not recognised")

        # sample["cov"].astype(np.float64)
        try:
            target = np.array(eval(sample["target"].replace('.', ', ')))
        except:
            t = re.sub("[ ]{1,}", ",", sample["target"])
            t = t.replace(',]', ']')
            t = t.replace('[,', '[')
            target = np.array(eval(t))
            # np.array(eval(re.sub(r'(?<!\d)(\s?)(,)(\s?)(?!\d)', r'\1\3',  sample["target"])))
        assert pred_cov.shape == (
            2, 2), "Should be only single prediction. Covariance matrix is not 2x2"
...
def generate_summary_df_withnlpd(ind_lms_results, sdr_dicts, nlpd_results, mean_error_results):
    """
    Generates pandas dataframe with summary statistics.
    Args:
        ind_lms_results [[]]: A 2D list of all lm errors. A list for each landmark
    ...
    Returns:pd.DataFrame.from_dict({(i,j): user_dict[i][j]
        ...
        PandasDataframe: A dataframe with statsitics including mean error, std error of All and individual
            ...
            df = {"Mean Er": {"All": 1, "L0": 1,...}, "std Er": {"All":1, "l0": 1, ...}, ...}
    """
    assert len(ind_lms_results) == 1, "only 1 landmark at a time for this evaluation funciton. Needs to be extended to multiple landmarks."
    results_dict = {"Error Mean": {}, "Error Std": {}, "NLPD Mean": {}, "NLPD Std": {},
                    "Spear_corr_nlpd_error": {},  "Spear_corr_nlpd_error_pvalue": {}, "pear_corr_nlpd_error": {}, "pear_corr_nlpd_error_pvalue": {}}
    for k in sdr_dicts.keys():
        results_dict["SDR" + str(k)] = {}
    ...
    results_dict["Error Mean"]["All"] = np.mean(only_w_anns)
    results_dict["Error Std"]["All"] = np.std(only_w_anns)
    results_dict["NLPD Mean"]["All"] = np.mean(list(nlpd_results.values()))
    results_dict["NLPD Std"]["All"] = np.std(list(nlpd_results.values()))

    all_nlpd = list(nlpd_results.values())
    all_errors = list(mean_error_results.values())

    spm_corr, spm_p = stats.spearmanr(
        all_nlpd, all_errors, alternative="greater")
    pear_corr, pear_p = stats.pearsonr(all_nlpd, all_errors)
    results_dict["Spear_corr_nlpd_error"]["All"] = spm_corr
    results_dict["Spear_corr_nlpd_error_pvalue"]["All"] = spm_p

    results_dict["pear_corr_nlpd_error"]["All"] = pear_corr
    results_dict["pear_corr_nlpd_error_pvalue"]["All"] = pear_p
