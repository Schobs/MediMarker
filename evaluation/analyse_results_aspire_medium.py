...
def analyse_all_folds(
    root_path, name_of_exp, models_to_test, early_stop_strategy, folds
):
    ...
    if skipped_folds == folds:
        ...
        for chkpt_key, results in individual_dicts.items():
            ...
            list_res = [
                {"uid": (x["uid"]), "ind_errors": x[all_lm_keys]}
...
for exp in all_exps:
    ...
    summary_dicts, failed_exp = analyse_all_folds(
        root_path, exp, models_to_test, early_stop_strategy, folds
    )
