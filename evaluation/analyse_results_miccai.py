...
def analyse_all_folds(
    ...
    """
    all_exps = [os.path.join(root_path, x) for x in os.listdir(root_path) if name_of_exp in x]

    matches_ind, summaries_ind = [sorted(x) for x in find_spreadsheets(all_exps, folds, landmark)]
    ...
    if skipped_folds == folds:
        ...
        for chkpt_key, results in individual_dicts.items():
            ...
            results['error_input_size'] = results['uid'].map(mean_error_results)
    ...
    with ExcelWriter(
        os.path.join(collation_location, name_of_exp + "_individual_L" + str(landmark)+".xlsx")
    ) as writer:
        for n, df in (individual_dicts).items():
            df.to_excel(writer, n, index=False)

    with ExcelWriter(
        os.path.join(collation_location, name_of_exp + "_summary_results_allfolds_L" + str(landmark)+".xlsx")
    ) as writer:
        for n, df in (summary_dicts).items():
...
for lm in landmarks:
    ...
    if not failed_exp:
        ...
        for chkpt_key, results in summary_dicts.items():
            ...
            relevant_results = relevant_results.rename(index=dict(zip(relevant_results.index, [exp_name])))
        ...
        failed_experiments.append([exp_name, summary_dicts])
