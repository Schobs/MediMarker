...
def ASPIRE_FOLLOWUP_resize(path_to_annotations, path_to_images, output_path_images, output_path_anno, modality, manual_omissions_uid, landmark_names, resize=[512,512], debug=False):
    ...
    for folder in all_folders:
        ...
        if ep_dcm_image.shape[0] != resize[0] or ep_dcm_image.shape[1] != resize[1]:
            ...
            downscale_factor = [ep_dcm_image.shape[0] / resize[0], ep_dcm_image.shape[1] / resize[1]]
            ...
            if inner_dict["has_annotation"]:
                ...
                for lm in all_lms:
                    resized_lms.append([lm[0] / downscale_factor[1], lm[1] / downscale_factor[0]])
        ...
        inner_dict['image'] = ((this_im_path.split("Follow-up")[-1]).split(".dcm")[0] + ".npz")[1:]

        print("what im sabving currently", inner_dict['image'])
        # exit()
        data['testing'].append(inner_dict)
        print("path adding to json: ",  ((this_im_path.split("Follow-up")[-1]).split(".dcm")[0] + ".npz")[1:])



        #save the new dicom

        save_path_this_im = output_path_images + (this_im_path.split("Follow-up")[-1]).split(".dcm")[0] + ".npz"
        ...
        if debug and need_resize:
            ...
            if inner_dict["has_annotation"]:
                ...
                for lm in all_lms:
                    rect1 = patches.Rectangle((int(lm[0]), int(lm[1])),3,3,linewidth=2,edgecolor='m',facecolor='none')
...
def dicom_to_npz(path_to_annotations, root_path, output_path_anno, modality, resize=[512,512], debug=False):
    ...
    for anno_path in all_anno_files:
        ...
        for split in ['training', 'validation', 'testing']:
            ...
            for sample in anno[split]:
                ...
                image = dicom.dcmread(os.path.join(root_path, sample["image"])).pixel_array
                ...
                save_path_this_im = os.path.join(root_path ,sample["image"].split(".dcm")[0] + ".npz")
                # make_this_dir = '/'.join(save_path_this_im.split("/")[:-1])
...
def resize_aspire_followup():
    for modality in ["4ch", "SA"]: #[4ch, "SA"]
        ...
        output_path_anno =prestring+"data/CMRI/PAH-Followup/landmark_localisation_annotations_resized/"+ modality

    
        ASPIRE_FOLLOWUP_resize(path_to_annotations, path_to_images, output_path_images, output_path_anno, modality, manual_omissions, landmarks, resize= [512, 512],debug=False )
...
def aspire_dicom_to_npz():
     for modality in ["4ch", "SA"]: #[4ch, "SA"]
        ...
        path_to_annotations = prestring+"data/CMRI/PAH-Baseline/Proc/landmark_localisation_annotations/"+ modality
        output_path_anno =prestring+"data/CMRI/PAH-Baseline/Proc/landmark_localisation_annotations_npz/"+ modality
        root_path = prestring+"data/CMRI/PAH-Baseline/Proc/" + modality
        dicom_to_npz(path_to_annotations,root_path, output_path_anno, modality, debug=True)
