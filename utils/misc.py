...
def set_determinism(
    ...
    additional_settings: Sequence[Callable[[int],
                                           Any]] | Callable[[int], Any] | None = None,
) -> None:
    """
    ...
    if torch.backends.flags_frozen():
        warnings.warn(
            "PyTorch global flag support of backends is disabled, enable it to set global `cudnn` flags.")
    ...
    if use_deterministic_algorithms is not None:
        ...
        if hasattr(torch, "use_deterministic_algorithms"):
            ...
            warnings.warn(
                "use_deterministic_algorithms=True, but PyTorch version is too old to set the mode.")
...
def copy_to_device(
    ...
    if verbose:
        ...
        warnings.warn(f"{fn_name} called with incompatible type: " +
                      f"{type(obj)}. Data will be returned unchanged.")
...
def str2bool(value: str | bool, default: bool = False, raise_exc: bool = True) -> bool:
    ...
    if raise_exc:
        raise ValueError(
            f"Got \"{value}\", expected a value from: {', '.join(true_set + false_set)}")
    return default
...
def str2list(value: str | list | None, raise_exc: bool = True) -> list | None:
    ...
    if value is None:
        ...
        raise ValueError(
            f'Unable to convert "{value}", expected a comma-separated str, e.g. 1,2,3')
...
def sample_slices(data: NdarrayOrTensor, dim: int = 1, as_indices: bool = True, *slicevals: int) -> NdarrayOrTensor:
    ...
    slices[dim] = slicevals if as_indices else slice(
        *slicevals)  # type: ignore
...
def check_parent_dir(path: PathLike, create_dir: bool = True) -> None:
    ...
    if not path_dir.exists():
        if create_dir:
            ...
            raise ValueError(
                f"the directory of specified path does not exist: `{path_dir}`.")
