...
from utils.trunet_networks.layers.utils import get_act_layer, get_dropout_layer, get_norm_layer
...
class ADN(nn.Sequential):
    ...
    def __init__(
        ...
        if norm is not None:
            ...
            op_dict["N"] = get_norm_layer(
                name=norm, spatial_dims=norm_dim or dropout_dim, channels=in_channels)
        ...
        if dropout is not None:
            if norm_dim is None and dropout_dim is None:
                raise ValueError(
                    "norm_dim or dropout_dim needs to be specified.")
            op_dict["D"] = get_dropout_layer(
                name=dropout, dropout_dim=dropout_dim or norm_dim)
