...
allow_missing_reference = os.environ.get(
    "MONAI_ALLOW_MISSING_REFERENCE", "0") != "0"
    ...
    for importer, name, is_pkg in walk_packages(
        ...
        if (is_pkg or load_all) and name not in sys.modules and match(exclude_pattern, name) is None:
            try:
                ...
                raise type(e)(f"{e}\n{msg}").with_traceback(
                    e.__traceback__) from e  # raise with modified message
...
def instantiate(__path: str, __mode: str, **kwargs: Any) -> Any:
    ...
    """
    from utils.enums import CompInitMode

    component = locate(__path) if isinstance(__path, str) else __path
    if component is None:
        raise ModuleNotFoundError(
            f"Cannot locate class or function path: '{__path}'.")
    ...
    try:
        ...
        if not callable(component):
            warnings.warn(
                f"Component {component} is not callable when mode={m}.")
            return component
    ...
    warnings.warn(
        f"Component to instantiate must represent a valid class or function, but got {__path}.")
    return component
...
def exact_version(the_module: Any, version_str: str = "", *_args: Any) -> bool:
    ...
    if not hasattr(the_module, "__version__"):
        warnings.warn(
            f"{the_module} has no attribute __version__ in exact_version check.")
        return False
...
def require_pkg(
    ...
    def _decorator(obj):
        ...
        _, has = optional_import(
            module=pkg_name, version=version, version_checker=version_checker)
...
def pytorch_after(major: int, minor: int, patch: int = 0, current_ver_string: str | None = None) -> bool:
    ...
    if c_mn != mn:
        return c_mn > mn
    is_prerelease = ("a" in f"{c_patch}".lower()) or (
        "rc" in f"{c_patch}".lower())
    c_p = 0
    try:
