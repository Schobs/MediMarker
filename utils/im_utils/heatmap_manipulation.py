        ...
        for hm_idx, hm in enumerate(hm_Stack):
            ...
            if visualize:
                ...
                ax[0].contour(
                    x,
                    y,
                    data_fitted_notheta.reshape(
                        predicted_heatmap.shape[0], predicted_heatmap.shape[1]
                    ),
                    8,
                    colors="w",
                )

                data_fitted_halftheta = twoD_Gaussian(
                    (x, y),
                    *[popt[0], popt[1], popt[2], popt[3], popt[4], popt[5] / 2, popt[6]]
                )
                ax[1].imshow(
                    predicted_heatmap,
                    cmap=plt.cm.jet,
                ...
                print(
                    "Covariance: Amplitude %s, coords: (%s,%s) sigmaXY (%s, %s), theta %s, offset %s "
                    % (
                        popt[0],
                        popt[1],
                        popt[2],
...
def candidate_smoothing(
    output,
    full_resolution,
    maxpool_factor,
    log_displacement_bool=True,
    smooth_factor=4,
    return_cropped_im=False,
    debug=False,
    save_fig=False,
):
    ...
    for lm in range(output[0].shape[0]):
        upscaled_hm = np.broadcast_to(
            predicted_heatmap[lm][:, None, :, None],
            (
                predicted_heatmap[lm].shape[0],
                step_size,
                predicted_heatmap[lm].shape[1],
                step_size,
            ),
        ).reshape(full_resolution)
        ...
        for x_idx, x in enumerate(range(0, full_resolution[0], step_size)):
            for y_idx, y in enumerate(range(0, full_resolution[1], step_size)):
                ...
                if (
                    0 <= loc[0] <= full_resolution[0]
                    and 0 <= loc[1] <= full_resolution[1]
                ):
                    all_locs.append(loc)
        ...
        if debug:
            ...
            coords_from_uhm, arg_max_uhm = get_coords(
                torch.tensor(
                    np.expand_dims(np.expand_dims(upscaled_hm, axis=0), axis=0)
                )
            )
